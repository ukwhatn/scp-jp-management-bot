...
from scp_jp.api.member_management import MemberManagementAPIClient, BatchStatusesResponseSchema, Status
...
class MemberManagement(commands.Cog):
    ...
    @group_system.command(name="batch_status", description="バッチ処理のステータスを確認します")
    @commands.is_owner()
    async def batch_status(self, ctx: discord.ApplicationContext):
        ...
        msg = "\n".join(
            f"{status.name}: {status.next_run_time.strftime('%Y-%m-%d %H:%M:%S')}" for status in result.statuses
        )
        await ctx.followup.send(msg)

    @group_system.command(name="run_batch", description="バッチ処理を強制実行します")
    @commands.is_owner()
    async def run_batch(self, ctx: discord.ApplicationContext, batch_name: discord.Option(str, "処理名")):
        await ctx.response.defer(ephemeral=True)
        try:
            ...
            if e.response.status_code == 404:
                ...
                return
            raise
        await ctx.followup.send(f":white_check_mark: バッチ処理 {batch_name} の強制実行リクエストを送信しました")

    # ==============================
    # 参加申請管理系
    # ==============================

    group_application = group_management.create_subgroup("application", "参加申請関連のコマンド")
    ...
    @group_application.command(name="add_notify_channel",
    ...
    async def toggle_notify_channel(
        ...
        if ctx.guild is None or ctx.channel is None:
            await ctx.followup.send(":x: サーバーチャンネル内で実行してください", ephemeral=True)
            return
        ...
        with db_session() as session:
            exist_entry = session.query(SiteApplicationNotifyChannel).filter_by(
            ...
            if exist_entry is None:
                # 新規作成
                session.add(SiteApplicationNotifyChannel(
                    guild_id=ctx.guild.id,
                    channel_id=ctx.channel.id,
                    site_id=site_id,
                ))
    ...
    @group_application.command(name="list_notify_channels", description="参加申請の通知先チャンネルの一覧を表示します")
    @commands.is_owner()
    async def list_notify_channels(self, ctx: discord.ApplicationContext):
        ...
        with db_session() as session:
            channels = session.query(SiteApplicationNotifyChannel).filter_by(guild_id=ctx.guild.id).all()
            msg = "\n".join([f"{ctx.guild.get_channel(channel.channel_id).mention} : {srv_sites[channel.site_id].name}"
        ...
        with db_session() as session:
            ...
            for channel in channels:
                ...
                for pending in pending_applications:
                    # original_idで検索
                    exist_entry = session.query(SiteApplication).filter_by(
                    ...
                    if exist_entry is None:
                        ...
                        if application_text is not None and pending.password is not None:
                            application_text.replace(pending.password, f"**`{pending.password}`**")
    ...
    @group_application.command(name="force_check", description="参加申請の強制チェックを行います")
    @commands.is_owner()
    async def force_check_site_applications(self, ctx: discord.ApplicationContext):
        await ctx.response.defer(ephemeral=True)
        self.check_site_applications.restart()
        await ctx.followup.send(":white_check_mark: 強制チェックを開始します", ephemeral=True)
